        ...
        DatabasesViewMsg dbMsg ->
            case dbMsg of
                DatabasesView.NavigateToDatabase dbName ->
                    ( model
                    , Effect.fromCmd (Nav.pushUrl shared.key (Route.routeToUrl (Route.ActivitiesRoute { db = dbName, name = Nothing, product = Nothing, limit = Just 20, classification = Nothing, classificationValue = Nothing })))
                    )
