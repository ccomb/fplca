...
- **`data/` has its own number, `data/VERSION`**, moved in the PR that changes a file under `data/`, and named in that PR's CHANGELOG entry ("data version 3") the way a wire revision is. The installers keep one `data/<version>/` per bundle and skip one they already hold, so a number left behind is a bundle nobody downloads; `scripts/check-data-version.sh` refuses it in CI. The engine's own number never moves for data.
