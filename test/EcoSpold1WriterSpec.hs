...
soloDb name prodU extra techs bios wastes =
  ...
  where
    ref = TechnosphereExchange prodU 1.0 kgUnit ReferenceProduct UUID.nil Nothing Nothing "" Nothing Nothing Nothing M.empty noProperties
...
linkedDb link =
  ...
  where
    ...
    refOf prodU = TechnosphereExchange prodU 1.0 kgUnit ReferenceProduct UUID.nil Nothing Nothing "" Nothing Nothing Nothing M.empty noProperties
    supplier = mkAct "aaa supplier" supU []
    -- The consumer's input consumes the supplier's product and links to it.
    consumer = mkAct "bbb consumer" conU [TechnosphereExchange supU 2.0 kgUnit Input link Nothing Nothing "" Nothing Nothing Nothing M.empty noProperties]
...
coproductDb =
  ...
  where
    ...
    mkAct nm prodU exs = Activity nm [] [] M.empty M.empty "GLO" LocationDeclared "kg" (refOf prodU : exs) M.empty M.empty Nothing Nothing Nothing
    refOf prodU = TechnosphereExchange prodU 1.0 kgUnit ReferenceProduct UUID.nil Nothing Nothing "" Nothing Nothing Nothing M.empty noProperties
    inputOnA = TechnosphereExchange prodAU 2.0 kgUnit Input supU Nothing Nothing "" Nothing Nothing Nothing M.empty noProperties
    ...
    describe "reference input (treatment process)" $
        it "rejects a ReferenceInput rather than flipping it to a reference product" $ do
            ...
            let prodU = read "77777777-0000-4000-8000-000000000001" :: UUID
                ...
                refInEx = TechnosphereExchange refInU 1.0 kgUnit ReferenceInput UUID.nil Nothing Nothing "" Nothing Nothing Nothing M.empty noProperties
