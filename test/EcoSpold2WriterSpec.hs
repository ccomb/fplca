  ...
  where
    activityA =
        Activity
            ...
            LocationDeclared
            "kg"
            [ TechnosphereExchange prodA 1.0 unitKg ReferenceProduct UUID.nil ClaimByProduct "" Nothing Nothing Nothing M.empty noProperties
            , TechnosphereExchange prodB 2.0 unitMJ Input actB ClaimByProduct "" (Just "energy input") Nothing Nothing M.empty noProperties
    ...
    activityB =
        Activity
            ...
            LocationDeclared
            "MJ"
            [ TechnosphereExchange prodB 1.0 unitMJ ReferenceProduct UUID.nil ClaimByProduct "" Nothing Nothing Nothing M.empty noProperties
  ...
  where
    ...
    activity =
        Activity
            ...
            "kg"
            [ TechnosphereExchange prodA 1.0 unitKg ReferenceProduct UUID.nil ClaimByProduct "" Nothing Nothing Nothing M.empty noProperties
            , TechnosphereExchange coprodU 0.4 unitKg Coproduct UUID.nil ClaimByProduct "" Nothing Nothing Nothing M.empty noProperties
            , WasteExchange wasteInU 0.2 unitKg True UUID.nil ClaimByProduct "" Nothing Nothing
            , WasteExchange wasteOutU 0.3 unitKg False UUID.nil ClaimByProduct "" Nothing Nothing
            ]
            M.empty
            M.empty
            (Just (EcoSpoldActivityType 1 "Ordinary transforming activity" Nothing Nothing))
...
spec = describe "EcoSpold2 writer round-trip" $ do
    ...
    describe "rejects un-encodable data at the export boundary" $ do
        ...
        it "rejects a reference input (no EcoSpold2 encoding)" $
            checkEcoSpold2Exportable
                (fixtureWithExchange (TechnosphereExchange co2 2.0 unitKg ReferenceInput UUID.nil ClaimByProduct "" Nothing Nothing Nothing M.empty noProperties))
                `shouldSatisfy` isLeft
