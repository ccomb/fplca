...
buildConversionDB = M.fromList $ concat
...
canConvert fromUnit toUnit = 
    fromUnit == toUnit || M.member (fromUnit, toUnit) buildConversionDB

-- | Get all possible target units for a given source unit
getConvertibleUnits :: Text -> [Text]  
getConvertibleUnits sourceUnit =
    [ targetUnit | ((from, targetUnit), _) <- M.toList buildConversionDB
    , from == sourceUnit
    ]
...
-- | Get conversion factor between two units (without applying to quantity)
getConversionFactor :: Text -> Text -> Maybe Double
getConversionFactor fromUnit toUnit
    | fromUnit == toUnit = Just 1.0
    | otherwise = M.lookup (fromUnit, toUnit) buildConversionDB

-- | List all EcoInvent unit types (categories)
getUnitTypes :: [Text]
getUnitTypes = 
    [ "area-time"
    , "area" 
    ...
    , "illuminance"
    , "length"  -- Note: EcoInvent XML has both "lenght" (typo) and "length"
    , "length-time"
    , "mass"    -- Not in XML list but implied by mass conversions
    , "person-transport-work"
    , "thermal-resistance"  -- Changed from "thermal insulance" (likely typo)
    , "time"
    , "volume"
    ]

-- | Get base unit for a unit type (the reference unit all others convert to)
getBaseUnit :: Text -> Maybe Text  
getBaseUnit unitType = case unitType of
    "mass" -> Just "kg"
    "energy" -> Just "MJ"
    "volume" -> Just "l"
    "area" -> Just "m2"
    "length" -> Just "m"
    "time" -> Just "s"
    "area-time" -> Just "m2*year"
    "length-time" -> Just "m*year"
    "density" -> Just "kg/m3"
    "freight-transport-work" -> Just "tkm"
    "person-transport-work" -> Just "pkm"
    "illuminance" -> Just "lux"
    "thermal-resistance" -> Just "m2*K/W"
    "currency" -> Just "EUR2005"  -- EcoInvent standard reference currency
    _ -> Nothing
...
unitsCompatible :: Text -> Text -> Bool
unitsCompatible unit1 unit2 = canConvert unit1 unit2 || canConvert unit2 unit1

-- | Debugging function to list all available conversions
listAllConversions :: [(Text, Text, Double)]
listAllConversions = 
    [ (from, to, factor) | ((from, to), factor) <- M.toList buildConversionDB ]

-- | Get all units of a specific type
getUnitsOfType :: Text -> [Text]
getUnitsOfType unitType = case unitType of
    "mass" -> ["kg", "g", "t", "tonne", "pound", "ounce avoirdupois"]
    "energy" -> ["MJ", "kJ", "GJ", "TJ", "Wh", "kWh", "cal", "kcal", "British thermal unit thermochemical"]
    "volume" -> ["l", "m3", "dm3", "cm3", "ml", "gallon fluid US"]
    "area" -> ["m2", "ha", "km2", "cm2", "mm2", "acre", "barn"]
    "length" -> ["m", "km", "cm", "mm", "micron", "mile", "inch", "ft", "yard"]
    "time" -> ["s", "min", "h", "d", "year", "a"]
    "area-time" -> ["m2*year", "ha*year", "acre*year"]
    "length-time" -> ["m*year", "km*year", "mile*year"]
    "density" -> ["kg/m3", "kg/l", "g/cm3", "g/l"]
    "freight-transport-work" -> ["tkm", "kg*km", "t*km", "pound*mile"]
    "person-transport-work" -> ["pkm", "person*km", "person*mile"]
    "illuminance" -> ["lux", "lm/m2", "lm/ft2"]
    "thermal-resistance" -> ["m2*K/W", "m2*°C/W", "ft2*°F*h/BTU"]
    _ -> []