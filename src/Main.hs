...
main = do
    ...
    if server
        then do
            ...
            putStrLn "    GET /api/v1/activity/{uuid}/tree?depth={n}      - Get activity supply chain tree (JSON)"
            putStrLn ""
            ...
            putStrLn ""
            putStrLn "  Count endpoints (get total count without data for pagination):"
            putStrLn "    GET /api/v1/search/flows/count?q={term}          - Count flows matching search"
            putStrLn "    GET /api/v1/search/flows/count?q={term}&lang={lang} - Count flows in specific language"
            putStrLn "    GET /api/v1/search/activities/count?name={term}   - Count activities matching criteria"
            putStrLn ""
