...
outputOpts = OutputOpts
    ...
    <*> option auto (long "level" <> short 'l' <> metavar "LEVEL" <> value 3
...
sourcesParser = SourcesMode
    <$> strOption (long "from-sources" <> short 's' <> metavar "FILE"
...
runSourcesMode configPath output level = do
    ...
    casGroups <- foldl' (M.unionWith S.union) M.empty <$>
    ...
    hPutStrLn stderr $ printf "Built: %d names, %d groups"
        (M.size (synNameToId db)) (M.size (synIdToNames db))

    -- Validate: no group > 100 names
    let bigGroups = [(gid, length names) | (gid, names) <- M.toList (synIdToNames db)
                                         , length names > 100]
    mapM_ (\(gid, sz) -> hPutStrLn stderr $
        printf "WARNING: Group %d has %d names (capped at 100)" gid sz) bigGroups
    ...
    parsePairRow row =
        ...
        in case fields of
            (a:b:_) ->
                ...
                in if T.null name1 || T.null name2 then Nothing
...
buildSynonymDB casGroups namePairs =
    let -- Step 1: Create initial groups from CAS
    ...
    in SynonymDB { synNameToId = nameToId, synIdToNames = idToNames }
...
mergeNamePairs pairs initialGroups =
    let -- Build name → group-id map from initial groups
    ...
    in map S.toList (M.elems finalGroupMap)
  where
        ...
        case (M.lookup name1 nameMap, M.lookup name2 nameMap) of
            ...
            (Just g1, Just g2) ->
                ...
                in if S.size merged > 100
                   ...
                   else let newNameMap = foldl' (\m n -> M.insert n g1 m) nameMap (S.toList s2)
            ...
            (Nothing, Nothing) ->
                ...
                in (M.insert name1 nid (M.insert name2 nid nameMap),
                    M.insert nid newGroup groupMap, nid + 1)
